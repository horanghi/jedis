...
public class CoordinatesPoint {
	...
	private class CoordRect {
		...
		public CoordRect(double minx, double miny, double maxx, double maxy) {
			this.minx = minx;
			this.miny = miny;
			this.maxx = maxx;
			this.maxy = maxy;
	...
	private final CoordRect[] rectArray1 = { new CoordRect(112500.0D, -50000.0D, 33500.0D, 53000.0D),
			new CoordRect(146000.0D, -50000.0D, 54000.0D, 58600.0D), new CoordRect(130000.0D, 44000.0D, 15000.0D, 14000.0D),
			new CoordRect(532500.0D, 437500.0D, 25000.0D, 25000.0D), new CoordRect(625000.0D, 412500.0D, 25000.0D, 25000.0D),
			new CoordRect(-12500.0D, 462500.0D, 17500.0D, 50000.0D) };
	private final CoordRect[] rectArray2 = { new CoordRect(112500.0D, -50000.0D, 33500.0D, 53000.0D),
			new CoordRect(146000.0D, -50000.0D, 54000.0D, 58600.0D), new CoordRect(130000.0D, 44000.0D, 15000.0D, 14000.0D),
			new CoordRect(532500.0D, 437500.0D, 25000.0D, 25000.0D), new CoordRect(625000.0D, 412500.0D, 25000.0D, 25000.0D),
			new CoordRect(-12500.0D, 462500.0D, 17500.0D, 50000.0D) };
	private double[][] deltaValue1 = { { 0.0D, 50000.0D }, { 0.0D, 50000.0D }, { 0.0D, 10000.0D }, { -70378.0D, -136.0D },
			{ -144738.0D, -2161.0D }, { 23510.0D, -111.0D } };
	private double[][] deltaValue2 = { { 0.0D, -50000.0D }, { 0.0D, -50000.0D }, { 0.0D, -10000.0D }, { 70378.0D, 136.0D },
			{ 144738.0D, 2161.0D }, { -23510.0D, 111.0D } };

	public CoordinatesPoint() {
		for (int i = 0; i < this.rectArray2.length; i++) {
			this.rectArray2[i].minx += this.deltaValue1[i][0];
			this.rectArray2[i].miny += this.deltaValue1[i][1];
		}
	}
	...
	public CoordinatesPoint(Point point) {
		this.x = point.getX();
		this.y = point.getY();
	}

	public CoordinatesPoint clone() {
		return new CoordinatesPoint(this.x, this.y);
	}
	...
	public void convertBESSEL2WGS() {
		...
		double[] rtn = GP2WGP(this.y, this.x, 0.0D, 6377397.155D, 0.0033427731799399794D);
		this.x = rtn[1];
		this.y = rtn[0];
	}
	...
	public void convertBESSEL2TM(double d, double e) {
		GP2TM(6377397.155D, 0.0033427731799399794D, 500000.0D, 200000.0D, 1.0D, e, d + 0.0028902777777777776D);
	}

	public void convertTM2BESSEL(double d, double e) {
		TM2GP(6377397.155D, 0.0033427731799399794D, 500000.0D, 200000.0D, 1.0D, e, d + 0.0028902777777777776D);
	}

	public void convertWGS2UTM(double d, double e) {
		setParameter(115.8D, -474.99D, -674.11D, 1.16D, -2.31D, -1.63D, -6.43D, 1.0D);
		GP2TM(6378137.0D, 0.0033528106647474805D, 0.0D, 500000.0D, 0.9996D, e, d);
	}

	public void convertWGS2WTM(double d, double e) {
		GP2TM(6378137.0D, 0.0033528106647474805D, 500000.0D, 200000.0D, 1.0D, e, d);
	}
	...
	public void convertUTM2WGS(double d, double e) {
		setParameter(115.8D, -474.99D, -674.11D, 1.16D, -2.31D, -1.63D, -6.43D, 1.0D);
		TM2GP(6378137.0D, 0.0033528106647474805D, 0.0D, 500000.0D, 0.9996D, e, d);
	}

	public void convertWGS2BESSEL() {
		setParameter(115.8D, -474.99D, -674.11D, 1.16D, -2.31D, -1.63D, -6.43D, 1.0D);
		double[] rtn = WGP2GP(this.y, this.x, 0.0D, 6377397.155D, 0.0033427731799399794D);
		this.x = rtn[1];
		this.y = rtn[0];
	}
	...
	public void convertWTM2WGS(double d, double e) {
		TM2GP(6378137.0D, 0.0033528106647474805D, 500000.0D, 200000.0D, 1.0D, e, d);
	}
	...
	private double[] WGP2WCTR(double a, double b, double d) {
		return GP2CTR(a, b, d, 6378137.0D, 0.0033528106647474805D);
	}
	...
	private double[] WCTR2WGP(double a, double b, double d) {
		return CTR2GP(a, b, d, 6378137.0D, 0.0033528106647474805D);
	}
	...
	private void GP2TM(double d, double e, double h, double g, double j, double l, double m) {
		...
		z = (d - z) / (d + z);
		E = d
				* (1.0D - z + 5.0D * (Math.pow(z, 2.0D) - Math.pow(z, 3.0D)) / 4.0D + 81.0D * (Math.pow(z, 4.0D) - Math.pow(z, 5.0D)) / 64.0D);
		I = 3.0D * d * (z - Math.pow(z, 2.0D) + 7.0D * (Math.pow(z, 3.0D) - Math.pow(z, 4.0D)) / 8.0D + 55.0D * Math.pow(z, 5.0D) / 64.0D)
				/ 2.0D;
		...
		J = G
				...
				* Math.pow(u, 5.0D)
				* j
				* (61.0D - 58.0D * Math.pow(A, 2.0D) + Math.pow(A, 4.0D) + 270.0D * w - 330.0D * Math.pow(A, 2.0D) * w + 445.0D
						* Math.pow(w, 2.0D) + 324.0D * Math.pow(w, 3.0D) - 680.0D * Math.pow(A, 2.0D) * Math.pow(w, 2.0D) + 88.0D
						* Math.pow(w, 4.0D) - 600.0D * Math.pow(A, 2.0D) * Math.pow(w, 3.0D) - 192.0D * Math.pow(A, 2.0D)
						* Math.pow(w, 4.0D)) / 720.0D;
		H = G * H * Math.pow(u, 7.0D) * j * (1385.0D - 3111.0D * Math.pow(A, 2.0D) + 543.0D * Math.pow(A, 4.0D) - Math.pow(A, 6.0D))
				/ 40320.0D;
		...
		w = G
				* Math.pow(u, 5.0D)
				* j
				* (5.0D - 18.0D * Math.pow(A, 2.0D) + Math.pow(A, 4.0D) + 14.0D * w - 58.0D * Math.pow(A, 2.0D) * w + 13.0D
						* Math.pow(w, 2.0D) + 4.0D * Math.pow(w, 3.0D) - 64.0D * Math.pow(A, 2.0D) * Math.pow(w, 2.0D) - 25.0D
						* Math.pow(A, 2.0D) * Math.pow(w, 3.0D)) / 120.0D;
	...
	private void TM2GP(double d, double e, double h, double g, double j, double l, double m) {
		...
		B = (d - B) / (d + B);
		G = d
				* (1.0D - B + 5.0D * (Math.pow(B, 2.0D) - Math.pow(B, 3.0D)) / 4.0D + 81.0D * (Math.pow(B, 4.0D) - Math.pow(B, 5.0D)) / 64.0D);
		E = 3.0D * d * (B - Math.pow(B, 2.0D) + 7.0D * (Math.pow(B, 3.0D) - Math.pow(B, 4.0D)) / 8.0D + 55.0D * Math.pow(B, 5.0D) / 64.0D)
				/ 2.0D;
		...
		I = E * (5.0D + 3.0D * Math.pow(E, 2.0D) + u - 4.0D * Math.pow(u, 2.0D) - 9.0D * Math.pow(E, 2.0D) * u)
				/ (24.0D * H * Math.pow(G, 3.0D) * Math.pow(j, 4.0D));
		J = E
				* (61.0D + 90.0D * Math.pow(E, 2.0D) + 46.0D * u + 45.0D * Math.pow(E, 4.0D) - 252.0D * Math.pow(E, 2.0D) * u - 3.0D
						* Math.pow(u, 2.0D) + 100.0D * Math.pow(u, 3.0D) - 66.0D * Math.pow(E, 2.0D) * Math.pow(u, 2.0D) - 90.0D
						* Math.pow(E, 4.0D) * u + 88.0D * Math.pow(u, 4.0D) + 225.0D * Math.pow(E, 4.0D) * Math.pow(u, 2.0D) + 84.0D
						* Math.pow(E, 2.0D) * Math.pow(u, 3.0D) - 192.0D * Math.pow(E, 2.0D) * Math.pow(u, 4.0D))
				/ (720.0D * H * Math.pow(G, 5.0D) * Math.pow(j, 6.0D));
		H = E * (1385.0D + 3633.0D * Math.pow(E, 2.0D) + 4095.0D * Math.pow(E, 4.0D) + 1575.0D * Math.pow(E, 6.0D))
		...
		u = (5.0D + 6.0D * u + 28.0D * Math.pow(E, 2.0D) - 3.0D * Math.pow(u, 2.0D) + 8.0D * Math.pow(E, 2.0D) * u + 24.0D
				* Math.pow(E, 4.0D) - 4.0D * Math.pow(u, 3.0D) + 4.0D * Math.pow(E, 2.0D) * Math.pow(u, 2.0D) + 24.0D * Math.pow(E, 2.0D)
				* Math.pow(u, 3.0D))
				/ (120.0D * Math.pow(G, 5.0D) * z * Math.pow(j, 5.0D));
		z = (61.0D + 662.0D * Math.pow(E, 2.0D) + 1320.0D * Math.pow(E, 4.0D) + 720.0D * Math.pow(E, 6.0D))
	...
	private void shiftIsland(boolean d) {
		...
		if (d) {
			for (int i = 0; i < this.rectArray1.length; i++) {
				if ((this.x - this.rectArray1[i].minx >= 0.0D) && (this.x - this.rectArray1[i].minx <= this.rectArray1[i].maxx)
						&& (this.y - this.rectArray1[i].miny >= 0.0D) && (this.y - this.rectArray1[i].miny <= this.rectArray1[i].maxy)) {
			...
			y = this.y / 2.5D;
			for (int i = 0; i < this.rectArray2.length; i++) {
				if ((x - this.rectArray2[i].minx >= 0.0D) && (x - this.rectArray2[i].minx <= this.rectArray2[i].maxx)
						&& (y - this.rectArray2[i].miny >= 0.0D) && (y - this.rectArray2[i].miny <= this.rectArray2[i].maxy)) {
